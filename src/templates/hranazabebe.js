...
const Hranazabebe = (props) => {

    const blogContent = props.data.allContentfulBlog
    const { currentPage, numPages } = props.pageContext
    const isFirst = currentPage === 1
    const isLast = currentPage === numPages
    const prevPage = currentPage - 1 === 1 ? '/category/hrana-za-bebe' : `/category/hrana-za-bebe/${currentPage - 1}`
    const nextPage = `/category/hrana-za-bebe/${currentPage + 1}`
...
export default Hranazabebe

export const pageQuery = graphql` 
 query HranazabebeQuery ($skip: Int!, $limit: Int!) {
   allContentfulBlog(
