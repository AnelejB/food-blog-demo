...
const Brzirucak = (props) => {

    const blogContent = props.data.allContentfulBlog
    const { currentPage, numPages } = props.pageContext
    const isFirst = currentPage === 1
    const isLast = currentPage === numPages
    const prevPage = currentPage - 1 === 1 ? '/category/brzi-rucak' : `/category/brzi-rucak/${currentPage - 1}`
    const nextPage = `/category/brzi-rucak/${currentPage + 1}`
...
export default Brzirucak

export const pageQuery = graphql` 
 query BrzirucakQuery ($skip: Int!, $limit: Int!) {
   allContentfulBlog(
